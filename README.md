...
Tampermonkey 脚本，基于 [CloudDrive2](https://www.clouddrive2.com/)（CD2）的 gRPC-web API 提供网页一键离线等增强功能。
...
在 [VCB-Studio](https://vcb-s.com/) 页面一键把项目种子添加到 CD2 离线下载任务，并在右上角实时跟踪任务状态。
...
2. [点此安装脚本](https://github.com/cyrahs/cd2-js/releases/latest/download/cd2-js.user.js)（或从 GreasyFork 安装）

## 配置

Tampermonkey 菜单 →「CloudDrive2 设置」：

1. 填写 CD2 地址（默认 `http://localhost:19798`）、API Token（CD2 管理界面生成）、离线目标目录（云盘内路径，如 `/115/离线`，目录所在云盘需支持离线下载）
2. 点「测试连接」确认后保存
...
发布：直接 push 到 main。CI 校验构建通过后自动发 GitHub Release（`@version` 由 git 提交数自动生成，tag 由 workflow 自动打），GreasyFork 通过 release webhook 即时同步。
